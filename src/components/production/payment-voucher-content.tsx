...
export function PaymentVoucherContent({
  ...
  const filteredPaymentVouchers = paymentVouchers.filter((voucher) => {
    ...
    const matchesParty =
      !partyFilter ||
      partyFilter === "all" ||
      voucher.ledger_id === partyFilter;

    const matchesDate =
      (!startDateFilter ||
        new Date(voucher.date) >= new Date(startDateFilter)) &&
      (!endDateFilter ||
    ...
    const matchesType =
      !typeFilter ||
      typeFilter === "all" ||
      voucher.payment_type === typeFilter;
  ...
  const handleDeletePaymentVoucher = async (voucherId: number) => {
    ...
    try {
      ...
      if (error) {
        showToast(
          "Failed to delete payment voucher. Please try again.",
          "error",
        );
        return;
      }
  ...
  const filteredStitchingChallans = stitchingChallans.filter((challan) => {
    ...
    const matchesLedger =
      !formData.ledger_id || challan.ledger_id === formData.ledger_id;
    ...
    const matchesSearch =
      ...
      challan.challan_no
        .toLowerCase()
        .includes(challanSearchTerm.toLowerCase()) ||
    ...
    try {
      ...
      if (paymentType === "Credit") {
        ...
        const { data: stitchingChallans } = await supabase
          ...
          .select(
            "id, challan_no, date, transport_charge, quality, batch_number",
          )
        ...
        const { data: weaverChallans } = await supabase
          .from("weaver_challans")
          .select(
            "id, challan_no, challan_date, total_grey_mtr, batch_number, quality_details",
          )
          .eq("ledger_id", ledgerId);
        ...
        if (weaverChallans) {
          transactions.push(
            ...weaverChallans.map((challan) => {
              const qualityName =
                Array.isArray(challan.quality_details) &&
  ...
  const validateForm = () => {
    ...
    if (hasCGST && hasIGST) {
      showToast(
        "You cannot select both CGST and IGST. Please choose either CGST or IGST",
        "error",
      );
      return false;
    }
  ...
  const selectedLedger = ledgers.find(
    (ledger) => ledger.ledger_id === formData.ledger_id,
  );
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Create a new payment voucher</p>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-gray-500">
                    Date is set to today&apos;s date and cannot be changed
                  </p>
                </div>
                ...
                <div className="space-y-2 md:col-span-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleChange("payment_for", e.target.value)
                    }
                ...
                <div className="space-y-2">
                  <Label htmlFor="transaction_to_settle">
                    Select Transaction to Settle
                  </Label>
                  <Select
                    value={formData.transaction_to_settle}
                    onValueChange={(value) =>
                      handleChange("transaction_to_settle", value)
                    }
                    ...
                    availableTransactions.length === 0 && (
                      <p className="text-sm text-gray-500">
                        ...
                        {formData.payment_type === "Credit"
                          ? "debit"
                          : "credit"}{" "}
                        transactions found for settlement
                      </p>
                    )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select
                      value={formData.sgst}
                      onValueChange={(value) => handleChange("sgst", value)}
                    >
                      <SelectTrigger
                        className={
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={formData.cgst}
                      onValueChange={(value) => handleChange("cgst", value)}
                    >
                      <SelectTrigger
                        className={
                      ...
                      <SelectContent className="bg-white">
                        ...
                        <SelectItem
                          value="2.5%"
                          disabled={formData.igst !== "Not Applicable"}
                        >
                          2.5%
                        </SelectItem>
                        <SelectItem
                    ...
                    {formData.igst !== "Not Applicable" && (
                      <p className="text-xs text-orange-500">
                        CGST disabled because IGST is selected
                      </p>
                    )}
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={formData.igst}
                      onValueChange={(value) => handleChange("igst", value)}
                    >
                      <SelectTrigger
                        className={
                      ...
                      <SelectContent className="bg-white">
                        ...
                        <SelectItem
                          value="2.5%"
                          disabled={formData.cgst !== "Not Applicable"}
                        >
                          2.5%
                        </SelectItem>
                        <SelectItem
                    ...
                    {formData.cgst !== "Not Applicable" && (
                      <p className="text-xs text-orange-500">
                        IGST disabled because CGST is selected
                      </p>
                    )}
                    ...
                    <div className="space-y-4">
                      {!formData.ledger_id ? (
                        <div className="text-center py-8 text-gray-500">
                          <p>
                            Please select a ledger first to view related
                            stitching challans
                          </p>
                        </div>
                      ) : (
                        <>
                          <Input
                            ...
                            onChange={(e) =>
                              setChallanSearchTerm(e.target.value)
                            }
                          />
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-2xl font-bold text-green-600">
              {formatCurrency(
                filteredPaymentVouchers.reduce(
                      ...
                      <div className="flex items-center">
                        <div className="w-8 h-8 bg-gray-100 rounded mr-2 flex items-center justify-center">
                          {voucher.ledgers?.business_logo ? (
                            ...
                            <span className="text-xs font-bold text-gray-600">
                              {voucher.ledgers?.business_name?.charAt(0) ||
                                "N/A"}
                            </span>
                          )}
                        </div>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="bg-white">
                          <DropdownMenuItem
                            onClick={() =>
                              router.push(
                                `/dashboard/production/payment-voucher/${voucher.id}`,
                              )
                            }
                          >
                          ...
                          {canEdit && (
                            <DropdownMenuItem
                              onClick={() =>
                                router.push(
                                  `/dashboard/production/payment-voucher/${voucher.id}/edit`,
                                )
                              }
                            >
                          ...
                          <DropdownMenuItem
                            onClick={() =>
                              router.push(
                                `/dashboard/production/payment-voucher/${voucher.id}/logs`,
                              )
                            }
                          >
                          ...
                          {canEdit && (
                            <DropdownMenuItem
                              ...
                              onClick={() =>
                                handleDeletePaymentVoucher(voucher.id)
                              }
