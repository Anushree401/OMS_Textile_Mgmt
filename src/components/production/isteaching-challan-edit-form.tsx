...
export function IsteachingChallanEditForm({
  ...
  const onSubmit = async (data: IsteachingChallanFormData) => {
    ...
    if (data.selected_sizes && availableSizes.length > 0) {
      const totalSelectedQty = data.selected_sizes.reduce(
        (sum, s) => sum + s.quantity,
        0,
      );
      const maxAvailableQty = availableSizes.reduce(
        (sum, s) => sum + s.quantity,
        0,
      );
      ...
      if (totalSelectedQty > maxAvailableQty) {
        setError(
          `Selected quantities (${totalSelectedQty}) cannot exceed available stock (${maxAvailableQty}).`,
        );
        setIsAlertOpen(true);
        setLoading(false);
        return;
      ...
      for (const selectedSize of data.selected_sizes) {
        ...
        if (availableSize && selectedSize.quantity > availableSize.quantity) {
          setError(
            `Quantity for size ${selectedSize.size} (${selectedSize.quantity}) cannot exceed available stock (${availableSize.quantity}).`,
          );
          setIsAlertOpen(true);
          setLoading(false);
          return;
    ...
    try {
      ...
      const { selected_sizes, ...finalUpdateData } =
        updateData as typeof updateData & { selected_sizes?: unknown };
      ...
      if (updateError) {
        ...
        setError(
          `Failed to update challan: ${updateError.message || "Unknown error"}`,
        );
      ...
      if (!onSuccess) {
        router.push(
          `/dashboard/production/isteaching-challan/${isteachingChallan.id}`,
        );
      }
    } catch (err) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-lg font-semibold">
                  {bothSelected ? bothPcsCreated : topPcsCreated}
                </p>
              </div>
            </div>
          )}
          ...
          {clothType?.includes("BOTTOM") && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-lg font-semibold">
                  {bothSelected ? bothPcsCreated : bottomPcsCreated}
                </p>
              </div>
            </div>
          )}
