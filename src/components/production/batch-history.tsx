...
export default function BatchHistory({ batchNumber }: BatchHistoryProps) {
  ...
  const totalShorting =
    history.shorting_entries?.reduce((sum, entry) => sum + entry.quantity, 0) ||
    0;

  const totalStitching =
    history.isteaching_challans?.reduce(
      (sum, challan) => sum + challan.quantity,
      0,
    ) || 0;

  const totalExpenses =
    history.expenses?.reduce((sum, expense) => sum + expense.amount, 0) || 0;
  ...
  const totalTopQty =
    history.isteaching_challans?.reduce(
      (sum, challan) =>
        sum + (challan.top_qty || 0) + (challan.both_top_qty || 0),
      0,
    ) || 0;

  const totalBottomQty =
    history.isteaching_challans?.reduce(
      (sum, challan) =>
        sum + (challan.bottom_qty || 0) + (challan.both_bottom_qty || 0),
      0,
    ) || 0;
  ...
  const manufacturingCostPerUnit =
    goodInventoryCount > 0 ? totalCost / goodInventoryCount : 0;
    ...
    <div className="container mx-auto py-6">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">
          Detailed information for batch{" "}
          <span className="font-semibold">{batchNumber}</span>
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-xs text-muted-foreground">After shorting</p>
          </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          ...
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-2xl">
                ₹{manufacturingCostPerUnit.toFixed(2)}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-xs text-muted-foreground">
      ...
      {history.isteaching_challans &&
        history.isteaching_challans.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardTitle className="text-2xl">
                  {(
                    totalStitching / history.isteaching_challans.length
                  ).toFixed(1)}{" "}
                  pcs
                </CardTitle>
              </CardHeader>
            ...
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardTitle className="text-2xl">
                  {initialQuantity > 0
                    ? ((totalStitching / initialQuantity) * 100).toFixed(1)
                    : "0"}
                  %
                </CardTitle>
              </CardHeader>
        ...
        {history.weaver_challan && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Initial batch creation details</CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    {history.isteaching_challans.map((challan, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell className="text-right">
                          ...
                          {challan.both_selected && challan.both_top_qty
                            ? `${challan.both_top_qty}m`
                            : ""}
                          {challan.top_pcs_qty
                            ? `/${challan.top_pcs_qty}pcs`
                            : ""}
                        </TableCell>
                        <TableCell className="text-right">
                          {challan.bottom_qty ? `${challan.bottom_qty}m` : ""}
                          {challan.both_selected && challan.both_bottom_qty
                            ? `${challan.both_bottom_qty}m`
                            : ""}
                          {challan.bottom_pcs_qty
                            ? `/${challan.bottom_pcs_qty}pcs`
                            : ""}
                        </TableCell>
                        <TableCell className="text-right">
                            ...
                            <Badge
                              className={
                                challan.inventory_classification === "good"
                                  ...
                                  : challan.inventory_classification === "bad"
