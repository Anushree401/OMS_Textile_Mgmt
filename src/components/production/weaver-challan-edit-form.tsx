...
const weaverChallanEditSchema = z.object({
  ...
  total_grey_mtr: z
    .number()
    .min(0.01, "Total grey meters must be greater than 0"),
  ...
  quality_details: z
    .array(qualityDetailSchema)
    .min(1, "At least one quality detail is required"),
  ...
  sgst: z
    .enum(["2.5%", "5%", "6%", "9%", "12%", "18%", "Not Applicable"])
    .optional(),
  cgst: z
    .enum(["2.5%", "5%", "6%", "9%", "12%", "18%", "Not Applicable"])
    .optional(),
...
export function WeaverChallanEditForm({
  weaverChallan,
  ledgers,
  userId,
  userName,
}: WeaverChallanEditFormProps) {
  ...
  const [selectedVendorLedger, setSelectedVendorLedger] =
    useState<Ledger | null>(
      ledgers.find((l) => l.ledger_id === weaverChallan.vendor_ledger_id) ||
        null,
    );
  ...
  const {
    ...
    defaultValues: {
      ...
      fold_cm: weaverChallan.fold_cm
        ? Number(weaverChallan.fold_cm)
        : undefined,
      width_inch: weaverChallan.width_inch
        ? Number(weaverChallan.width_inch)
        : undefined,
      taka: weaverChallan.taka,
      transport_name: weaverChallan.transport_name || "",
      lr_number: weaverChallan.lr_number || "",
      transport_charge: weaverChallan.transport_charge
        ? Number(weaverChallan.transport_charge)
        : undefined,
      ...
      vendor_amount: weaverChallan.vendor_amount
        ? Number(weaverChallan.vendor_amount)
        : undefined,
      // ... existing code ...
  ...
  const gstCalculation = {
    ...
    totalGST:
      calculateGSTAmount(sgst, vendorAmount || 0) +
      calculateGSTAmount(cgst, vendorAmount || 0) +
      calculateGSTAmount(igst, vendorAmount || 0),
    totalAmount:
      (vendorAmount || 0) +
      calculateGSTAmount(sgst, vendorAmount || 0) +
      calculateGSTAmount(cgst, vendorAmount || 0) +
  ...
  const calculateTotalGreyMtr = () => {
    const total = qualityDetails.reduce(
      (sum, detail) => sum + (detail.grey_mtr || 0),
      0,
    );
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select party from ledger or enter custom party details
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <LedgerSelectModal
              ledgers={ledgers}
              onLedgerSelect={handleVendorLedgerSelect}
            >
              <Button
                type="button"
                variant="outline"
              ...
              >
                {selectedVendorLedger
                  ? selectedVendorLedger.business_name
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <Select
                onValueChange={(value) =>
                  setValue("sgst", value as GSTPercentage)
                }
                defaultValue={weaverChallan.sgst || "Not Applicable"}
              >
                <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Select
                onValueChange={(value) =>
                  setValue("cgst", value as GSTPercentage)
                }
                defaultValue={weaverChallan.cgst || "Not Applicable"}
              >
                <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Select
                onValueChange={(value) =>
                  setValue("igst", value as GSTPercentage)
                }
                defaultValue={weaverChallan.igst || "Not Applicable"}
              >
                <SelectTrigger>
        ...
        <CardContent>
          {fields.map((field, index) => (
            <div key={field.id} className="flex items-center space-x-2 mb-2">
              <Select
                onValueChange={(value) =>
                  setValue(`quality_details.${index}.quality_name`, value)
                }
              ...
              <Input
                type="number"
                step="0.01"
                {...register(`quality_details.${index}.rate`, {
                  valueAsNumber: true,
                })}
                placeholder="Rate"
              />
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Update production measurements and specifications
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {takaFields.map((field, index) => (
            <div key={field.id} className="flex items-center space-x-2 mb-2">
              ...
              <Input
                type="number"
                step="0.01"
                {...register(`taka_details.${index}.meters`, {
                  valueAsNumber: true,
                })}
                placeholder="Meters"
              />
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Update transport and logistics information (optional)
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div className="flex items-center gap-4">
        ...
        <Button
          ...
          onClick={() =>
            router.push(
              `/dashboard/production/weaver-challan/${weaverChallan.id}`,
            )
          }
        >
          Cancel
