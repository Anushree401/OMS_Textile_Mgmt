...
export function ProductionDashboardContent({
    ...
    return batchData.filter((batch) => {
      ...
      switch (activeTab) {
        ...
        case "in-pipeline":
          ...
          tabFilterPassed =
            batch.stitching_challans.length === 0 ||
            batch.stitching_challans.some(
              (challan) =>
        ...
        case "completed":
          ...
          tabFilterPassed = batch.stitching_challans.some(
            (challan) => challan.inventory_classification === "good",
          );
          break;
      }
      ...
      // Party filter
      const partyFilterPassed =
        partyFilter === "all" || batch.weaver_challan_party === partyFilter;
      ...
      const statusFilterPassed =
        statusFilter === "all" || batchStatus === statusFilter;
  ...
  const totalStock = finishedStock.reduce(
    (sum, stock) => sum + stock.available_qty,
    0,
  );
  const totalQualities = finishedStock.length;
  const totalIssued = finishedStock.reduce(
    (sum, stock) => sum + stock.issued_qty,
    0,
  );
  const totalReceived = finishedStock.reduce(
    (sum, stock) => sum + stock.total_qty,
    0,
  );
  const totalShorted = finishedStock.reduce(
    (sum, stock) => sum + stock.shorted_qty,
    0,
  );
  const topStockQuality = finishedStock.reduce(
    (max, stock) => (stock.available_qty > max.available_qty ? stock : max),
    { quality_name: "N/A", available_qty: 0 },
  );
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              <Select
                value={statusFilter}
                onValueChange={(value) =>
                  setStatusFilter(value as "all" | "in-pipeline" | "completed")
                }
              >
                <SelectTrigger className="w-full md:w-48">
                  ...
                  <TableRow key={batch.batch_number}>
                    ...
                    <TableCell>
                      <div className="flex items-center text-sm">
                        ...
                        {new Date(
                          batch.weaver_challan_date,
                        ).toLocaleDateString()}
                      </div>
                    </TableCell>
                    <TableCell>{batch.weaver_challan_party}</TableCell>
                    <TableCell className="text-right">
                      {batch.weaver_challan_quantity}
                    </TableCell>
                    <TableCell>
                      <div
                    ...
                    <TableCell className="text-right">
                      <Button
                        ...
                        onClick={() =>
                          router.push(
                            `/dashboard/production/batch/${batch.batch_number}`,
                          )
                        }
                      >
                        View Details
          ...
          {paginatedBatches.length > 0 && (
            <div className="flex items-center justify-between mt-4">
              <div className="text-sm text-gray-500">
                ...
                to {Math.min(currentPage * itemsPerPage, sortedBatches.length)}{" "}
                of {sortedBatches.length} results
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              Total units issued for stitching
            </p>
          </CardContent>
        </Card>
        <Card>
