...
export default async function PaymentVoucherLogsPage({
  ...
  // Explicitly cast the logs data to the row array type
  const typedLogs = (logs || []) as PaymentVoucherLogRow[];

  const userIds = [
    ...new Set(typedLogs.map((log) => log.changed_by).filter(Boolean)),
  ] as string[];
  ...
  // FIX: Create map safely
  const usersMap = new Map(users.map((u) => [u.id, u]));

  // 7. Enhance logs with changer information
  const enhancedLogs: EnhancedLog[] = typedLogs.map((log) => {
    const changer = log.changed_by ? usersMap.get(log.changed_by) : null;
    ...
    return {
      ...
      changed_at: (log.changed_at || new Date().toISOString()) as string,
      id: (log.id || 0) as number,
      payment_voucher_id: (log.payment_voucher_id || 0) as number,
