...
// Define your log row and profile row types (edit per schema!)
type ExpenseLogRow = {
  id: string;
  changed_by: string;
  changed_at: string;
...
export default async function ExpenseLogsPage({
  ...
  // Always cast result types to avoid 'never' errors
  const { data: logsRaw, error: logsError } = await supabase
    .from("expense_logs")
    .select("*")
    .eq("expense_id", params.id)
    .order("changed_at", { ascending: false });
  const logs = (logsRaw as ExpenseLogRow[]) || [];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center space-x-4">
        <Link href={`/dashboard/production/expense/${params.id}`}>
                  ...
                  {Object.entries(log.changes || {}).map(([key, value]) => (
                    <div key={key} className="text-sm">
                      ...
                      <span className="text-red-500 line-through">
                        {" "}
                        {JSON.stringify((value as { old: unknown }).old)}
                      </span>{" "}
                      →
                      <span className="text-green-500">
                        {" "}
                        {JSON.stringify((value as { new: unknown }).new)}
                      </span>
                    </div>
                  ))}
                </div>
