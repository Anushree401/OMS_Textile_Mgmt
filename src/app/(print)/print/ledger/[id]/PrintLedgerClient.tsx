...
interface LedgerData {
  ...
  challans: Pick<
    ...
    | "igst"
  >[];
  paymentVouchers: Pick<
    PaymentVoucher,
    "id" | "date" | "payment_for" | "payment_type" | "amount"
  >[];
}
...
export default function PrintLedgerClient({
  ledgerData,
}: {
  ...
  useEffect(() => {
    ...
    const processTransactions = () => {
      ...
      const sortedVouchers = [...ledgerData.paymentVouchers].sort(
      ...
      const paymentVoucherTransactions = ledgerData.paymentVouchers.map(
        (pv) => {
      ...
      <div className="max-w-4xl mx-auto bg-white p-8 my-8 print:my-0 print:shadow-none shadow-lg font-sans">
          ...
          <div className="text-right">
            ...
            <p className="text-xs text-gray-500 mt-1">
              ...
              <span className="font-medium text-gray-700">
                {ledgerData.ledger.ledger_id}
              </span>
            </p>
            <p className="text-xs text-gray-500">
              Print Date:{" "}
              <span className="font-medium text-gray-700">
                {formatDateDateOnly(new Date().toISOString())}
              </span>
            </p>
          </div>
        </header>
        ...
        <section className="flex justify-between gap-8 mt-8 print:break-inside-avoid">
          <div className="flex-1">
            ...
            <div className="text-sm">
              <p className="font-bold text-gray-800 text-lg">
                {ledgerData.ledger.business_name}
              </p>
              {ledgerData.ledger.contact_person_name && (
                <p className="mt-1">
                  Contact: {ledgerData.ledger.contact_person_name}
                </p>
              )}
              {ledgerData.ledger.mobile_number && (
                <p>M: {ledgerData.ledger.mobile_number}</p>
              )}
              {ledgerData.ledger.email && <p>E: {ledgerData.ledger.email}</p>}
              {ledgerData.ledger.gst_number && (
                <p className="font-medium">
                  GST: {ledgerData.ledger.gst_number}
                </p>
              )}
            </div>
          </div>
          <div className="flex-1">
            ...
            <div className="text-sm">
              {ledgerData.ledger.address && <p>{ledgerData.ledger.address}</p>}
              <p className="mt-1">
                {[
        ...
        <section className="mt-8">
          ...
          <table className="w-full text-sm border-collapse">
            <thead className="bg-gray-100">
              <tr>
                <th className="border border-gray-300 p-2 text-left font-semibold text-gray-600">
                  S.NO
                </th>
                <th className="border border-gray-300 p-2 text-left font-semibold text-gray-600">
                  DATE
                ...
                <th className="border border-gray-300 p-2 text-right font-semibold text-gray-600">
                  CREDIT (₹)
                </th>
                <th className="border border-gray-300 p-2 text-right font-semibold text-gray-600">
                  DEBIT (₹)
              ...
              {transactions.map((transaction, index) => (
                <tr key={index} className="border-b">
                  ...
                  <td className="border border-gray-300 p-2">
                    {formatDateDateOnly(transaction.date)}
                  </td>
                  <td className="border border-gray-300 p-2">
                    {transaction.detail}
                  </td>
        ...
        <footer className="text-center mt-16 pt-4 text-xs text-gray-500 border-t">
          <p className="italic">
            * This is a system generated statement and does not require a
            signature.
          </p>
          <p className="mt-1">
            Printed on {formatDateDateOnly(new Date().toISOString())}
          </p>
        </footer>
      </div>
    </div>
